...
# mfcc默认提取出来的一帧13个特征
num_features = 13
# 26個英文單字 + 1个空白 + 1个no label = 28 label个数
...
def get_audio_feature(wav_filename):
  '''
  ...
  #提取mfcc特征
  inputs = mfcc(audio, samplerate=fs, nfft=1103)
  # 对特征数据进行归一化，减去均值除以方差
...
def get_audio_label(label_name):
  '''
  将label文本转换成整数序列，然后再换成稀疏三元组
  '''
  target_filename = label_name
...
def main():
  ...
  with tf.Session(config=config) as session:
    ...
    for wavnum in range(100):
      if wavnum<9:
        wavcount = "00" + str(wavnum+1)
      elif wavnum>=9 and wavnum<99:
        wavcount = "0" + str(wavnum+1)
      else:
        wavcount = "" + str(wavnum+1)
      for curr_epoch in range(num_epochs):
        ...
        for batch in range(num_batches_per_epoch):
          ...
          people_name = "00" + str(batch+1)
          fname = "../audio/wav" + people_name + "/" + people_name + "_" + wavcount + ".wav"
          train_inputs, train_seq_len = get_audio_feature(fname)
          # 获取这个样本的label
          label_n = "label_" + wavcount + ".txt"
          train_targets = get_audio_label(label_n)
      ...
      test_wav = "008_" + wavcount + ".wav"
      test_inputs, test_seq_len = get_audio_feature(test_wav)
      test_label = "label_" + wavcount + ".txt"
      test_targets = get_audio_label(test_label)
